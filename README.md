...
### 3. Feature Toggles | 功能开关

#### `ENABLE_DIFFICULTY_PROMPT`

When enabled, ProxyRouter automatically appends a difficulty indicator to the system instruction, so the AI acknowledges the routing decision:

| Model | Appended Instruction |
|-------|---------------------|
...
- **Cloudflare**: Go to Worker -> **Settings** -> **Variables** -> Add a Variable named `ENABLE_DIFFICULTY_PROMPT` with value `"false"`.
- **Vercel**: Go to Project -> **Settings** -> **Environment Variables** -> Add `ENABLE_DIFFICULTY_PROMPT` with value `"false"`.

启用后，ProxyRouter 会自动在系统指令中追加难度标识，让 AI 知道路由结果：
...
- **Cloudflare**: 进入 Worker -> **设置** -> **变量** -> 添加变量 `ENABLE_DIFFICULTY_PROMPT`，值为 `"false"`。
- **Vercel**: 进入项目 -> **Settings** -> **Environment Variables** -> 添加 `ENABLE_DIFFICULTY_PROMPT`，值为 `"false"`。
